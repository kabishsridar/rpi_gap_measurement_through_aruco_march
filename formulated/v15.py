...
High-legibility dark UI with massive status readouts and multi-axis tilt monitoring.
"""
...
# Status colors for larger chips
CHIP = {
    "ok":    (C_GREEN, C_BG,   "  OK  "),
...
def rotation_to_euler(R):
    ...
    if sy > 1e-6:
        # Standard decomposition: Yaw is rotation about Z, Pitch about Y, Roll about X
...
class MeasurementApp:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  TAB 2 — WARNINGS (PROFESSIONAL MONITOR)
    # ══════════════════════════════════════════════════════════════════════════
    def _build_tab_warnings(self):
        ...
        # Header Status
        hh = tk.Frame(main, bg=C_BG)
        hh.pack(fill="x", pady=(0, 20))
        self.overall_frame = tk.Frame(hh, bg=C_MUTED, padx=30, pady=15)
        self.overall_frame.pack(side="left")
        self.overall_lbl = tk.Label(self.overall_frame, text="SYSTEM INITIALIZING", font=("Helvetica", 20, "bold"), fg=C_BG, bg=C_MUTED)
        self.overall_lbl.pack()

        # Thresholds (expanded range to 180)
        tc = tk.Frame(hh, bg=C_BG)
        tc.pack(side="right")
        def _slider(parent, lbl, var, col):
            f = tk.Frame(parent, bg=C_SURF, padx=15, pady=10)
        ...
        _slider(tc, "PITCH LIMIT", self.pitch_threshold, C_AMBER)
        _slider(tc, "YAW LIMIT", self.yaw_threshold, C_TOP)

        # Diagnostics Grid
        dg = tk.Frame(main, bg=C_BG)
        dg.pack(fill="both", expand=True)
        self._warn_refs = {}
        ...
        for i, (key, label, accent) in enumerate([("top", "UPPER SENSORS", C_TOP), ("bottom", "LOWER SENSORS", C_BOT)]):
            ...
            for k2, desc in [("L_det", "LEFT Detected"), ("R_det", "RIGHT Detected"), ("rot", "In-Plane Rotation"), 
                ...
                chip, val = self._chip_row(sec, desc)
                refs[k2] = (chip, val)
            ...
            for ak, al, ac in [("L_pitch", "L-P", C_AMBER), ("L_yaw", "L-Y", C_TOP), ("L_roll", "L-R", C_MUTED),
                ...
                tk.Label(c, textvariable=s, font=("Courier", 16, "bold"), fg=C_TEXT, bg=C_SURF2).pack()
                av[ak] = s
            refs["_ang"] = av
            self._warn_refs[key] = refs
    ...
    def _build_tab_tele(self):
        ...
        m = tk.Frame(tab, bg=C_BG, padx=40, pady=20)
        ...
        for key, lab, acc in [("top", "UPPER TRIANGULATION", C_TOP), ("bottom", "LOWER TRIANGULATION", C_BOT)]:
            ...
            for vk, vl in v:
                ...
                tk.Label(r, textvariable=sv, font=("Courier", 14, "bold"), fg=C_TEXT, bg=C_SURF2).pack(side="right", padx=15)
                self.tele_vars[key][vk] = sv
    ...
    def _build_tab_settings(self):
        ...
        f = tk.Frame(tab, bg=C_BG, padx=100, pady=50)
        f.pack(fill="both", expand=True)

        rs = self._section(f, "Reference Orientation", fill="x", pady=20)
        rb = tk.Frame(rs, bg=C_SURF); rb.pack()
        for c in ["Left", "Right"]:
            tk.Radiobutton(rb, text=c, variable=self.fixed_side, value=c, bg=C_SURF, fg=C_TEXT, selectcolor=C_BLUE, 
                           indicatoron=0, font=("Helvetica", 20, "bold"), padx=60, pady=20, relief="flat").pack(side="left", padx=10)

        def _sb(title, var, acc, lo, hi):
            s = self._section(f, title, accent=acc, fill="x", pady=10)
            r = tk.Frame(s, bg=C_SURF); r.pack(fill="x")
    ...
    def _build_tab_cam(self):
        ...
        m = tk.Frame(tab, bg=C_BG, padx=20, pady=20)
        m.pack(fill="both", expand=True)
        
        pv = self._section(m, "Optical Stream", side="left", fill="both", expand=True, padx=(0, 20))
        self.cam_canvas = tk.Canvas(pv, width=800, height=480, bg="black", highlightthickness=0)
        self.cam_canvas.pack(expand=True)
        self.lbl_cam_status = tk.Label(pv, text="DETECTION MODE ACTIVE", font=("Helvetica", 14, "bold"), fg=C_GREEN, bg=C_SURF)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  Logic (Optimized for Industrial Loop)
    # ══════════════════════════════════════════════════════════════════════════
    def _mv_start(self):
    ...
    def _mv_tick(self):
        ...
        if self.mv_state == "collecting_init":
            ...
            if n >= COLLECT_N:
                ...
                self.mv_delta_lbl_top.config(text=f"{dt:+.3f}")
                self.mv_delta_lbl_bot.config(text=f"{db:+.3f}")
    ...
    def measurement_loop(self):
        ...
        log.init_log(); buf = {"top":[], "bottom":[]}; l_u = time.time()*1000
        ...
        while self.is_running:
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    c = corners[i][0]; iy = np.argsort(c[:,0]); lp, rp = c[iy[:2]], c[iy[2:]]
                    tl = lp[np.argmin(lp[:,1])]; bl = lp[np.argmax(lp[:,1])]
                    tr = rp[np.argmin(rp[:,1])]; br = rp[np.argmax(rp[:,1])]
                    m_list.append({"c": np.array([tl, tr, br, bl], dtype=np.float32), "c_raw": c, "y": np.mean(c[:,1]), "x": np.mean(c[:,0])})
                
                m_list.sort(key=lambda x: x["y"])
                sets = {"top": m_list[:2], "bottom": m_list[2:4] if len(m_list)>=4 else []}
                
                def _proc(ml, pk, sz):
                    if len(ml)<2: self.last_data[pk].update({"dist":0, "L_det":False, "R_det":False}); return
                    ml.sort(key=lambda x: x["x"])
                    is_r = self.fixed_side.get()=="Right"; Sm, Tm = (ml[1], ml[0]) if is_r else (ml[0], ml[1])
                    h = sz/2; obj = np.array([[-h,h,0],[h,h,0],[h,-h,0],[-h,-h,0]], dtype=np.float32)
                    
                    def _gp(cr):
                        _, rv, tv = cv.solvePnP(obj, cr, K, dc); R, _ = cv.Rodrigues(rv)
                        p, y, r = rotation_to_euler(R)
                        return np.array([R@pt+tv.ravel() for pt in obj]), float(tv[2]), p, y, r
                    
                    Spts, Sz, Sp, Sy, Sr = _gp(Sm["c_raw"]); Tpts, Tz, Tp, Ty, Tr = _gp(Tm["c_raw"])
                    irt = lambda cs: abs(math.degrees(math.atan2(cs[1,1]-cs[0,1], cs[1,0]-cs[0,0])))
                    Sir, Tir = irt(Sm["c"]), irt(Tm["c"])
                    
                    def _io(p, ir):
                        o = p[:,0].argsort(); i_ = p[o[2:]] if ir else p[o[:2]]; ou = p[o[:2]] if ir else p[o[2:]]
                        return i_[i_[:,1].argsort()][0], i_[i_[:,1].argsort()][1], ou[ou[:,1].argsort()][0]

                    St, Sb, So = _io(Spts, not is_r); Ta, _, _ = _io(Tpts, is_r)
                    A = (St+Sb)/2; X = (Ta+Ta)/2 # simplified for loop logic
                    ...
                    buf[pk].append({"A":A, "X":X, "TR":St, "TL":So, "BR":Sb, "dist":np.linalg.norm(X-A), "rot":max(Sir,Tir), "Lp":Sp, "Ly":Sy, "Lr":Sr, "Rp":Tp, "Ry":Ty, "Rr":Tr, "Lz":Sz, "Rz":Tz, "p1":p1, "p2":p2})
                ...
                if (curr - l_u) >= 1000:
                    for k in ["top", "bottom"]:
                        if buf[k]:
                            s = buf[k]; self.last_data[k].update({"dist": np.mean([x["dist"] for x in s]), "rot_2d": np.mean([x["rot"] for x in s]), "L_pitch": np.mean([x["Lp"] for x in s]), "L_yaw": np.mean([x["Ly"] for x in s]), "R_pitch": np.mean([x["Rp"] for x in s]), "R_yaw": np.mean([x["Ry"] for x in s]), "L_z": np.mean([x["Lz"] for x in s]), "R_z": np.mean([x["Rz"] for x in s]), "p1_px": s[-1]["p1"], "p2_px": s[-1]["p2"], "k": 0.5})
                            buf[k] = []; self.last_data["session_count"] += 1
                    l_u = curr
            else:
                for k in ["top", "bottom"]: self.last_data[k].update({"dist":0, "L_det":False, "R_det":False})

            for k, c in [("top",(50,150,250)), ("bottom",(200,100,255))]:
                d = self.last_data[k]
                if d["dist"]>0 and d["p1_px"]:
                    cv.line(frame, d["p1_px"], d["p2_px"], c, 4)
                    cv.putText(frame, f"{d['dist']:.2f}", d["p1_px"], 0, 1.2, c, 3)

            rgb = cv.cvtColor(frame, cv.COLOR_BGR2RGB); self.current_frame = rgb; self._cam_preview_frame = rgb
    ...
    def update_gui_loop(self):
        ...
        for pk in ["top", "bottom"]:
            ...
            ang = r["_ang"]
            ang["L_pitch"].set(f"{d['L_pitch']:+.1f}"); ang["L_yaw"].set(f"{d['L_yaw']:+.1f}"); ang["L_roll"].set(f"{d['L_A'][0]:+.1f}")
