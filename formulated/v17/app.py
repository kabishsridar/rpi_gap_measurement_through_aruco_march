...
class MeasurementApp:
    ...
    def _build_tab_settings(self):
        ...
        def _update(*_):
            ...
            c.create_oval(4, 4, 76, 36, fill=bg, outline="")
            cx = 58 if on else 22
            c.create_oval(cx-14, 6, cx+14, 34, fill="white", outline="")
    ...
    def update_gui_loop(self):
        if self.current_frame is not None:
            try: 
                ...
                img = ImageTk.PhotoImage(Image.fromarray(cv.cvtColor(f960, cv.COLOR_BGR2RGB)))
        ...
        if self._cam_preview_frame is not None:
            try: 
                ...
                img2 = ImageTk.PhotoImage(Image.fromarray(cv.cvtColor(f800, cv.COLOR_BGR2RGB)))
        ...
        while self.is_running:
            ...
            if ids is not None and len(ids) >= 1:
                ...
                for i in range(len(ids)):
                    c = corners[i][0]
                    m_data.append({"c": c, "y": np.mean(c[:, 1]), "x": np.mean(c[:, 0]), "c_raw": corners[i][0]})
                ...
                def proc(pair, pk, size):
                    if len(pair) < 2: 
                        self.last_data[pk]["dist"] = 0.0
                        self.last_data[pk]["L_det"] = False
                        return
                    ...
                    s_pts, s_roll, s_z, s_pitch, s_yaw = get_p(s_m["c_raw"])
                    t_pts, t_roll, t_z, t_pitch, t_yaw = get_p(t_m["c_raw"])
                    s_irot = self.engine.inplane_rot(s_m["c"])
                    t_irot = self.engine.inplane_rot(t_m["c"])
                    
                    sit, sib, sot = self.engine.inner_outer(s_pts, not is_rf)
                    tit, tib, _ = self.engine.inner_outer(t_pts, is_rf)
                    ...
                    self.last_data[pk]["L_det"] = True
                    self.last_data[pk]["R_det"] = True
                ...
                for key in ["top", "bottom"]:
                    if buffers[key] and (curr - l_u) >= 200:
                        ...
                        v_r = aTR-aTL
                        v = v_r/np.linalg.norm(v_r) if np.linalg.norm(v_r)>0 else np.zeros(3)
                        ...
                        if self.use_angle_thresh.get() and np.mean([x["rot"] for x in s]) > self.rot_threshold.get(): 
                        ...
                        log.record(dv, kv, aA, aX, aTR, np.mean([x["BR"] for x in s], axis=0), aB, aC, aB-aA, aTR-aTL, aC-aB, self.last_data[key]["L_A"], self.last_data[key]["R_A"])
                        buffers[key].clear()

            for k, color in [("top", (0,165,255)), ("bottom", (255,0,255))]:
                d = self.last_data[k]
                if d["dist"]>0 and d["p1_px"]:
                    X_3 = np.array([d["X"]], dtype=np.float64).reshape(1,1,3)
                    p2 = d["p2_px"]
                    if d["X"][2]>0:
                        p_p, _ = cv.projectPoints(X_3, np.zeros(3), np.zeros(3), K, dist_c)
                        p2 = tuple(p_p[0].ravel().astype(int))
                    cv.line(frame, d["p1_px"], p2, color, 3)
