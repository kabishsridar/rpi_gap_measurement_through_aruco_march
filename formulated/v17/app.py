...
class MeasurementApp:
    def __init__(self, root):
        ...
        # Start measurement thread with copy-pasted logic
    ...
    def _build_tab_live(self):
        ...
        ws = tk.Scrollbar(warn_f, orient="vertical", command=self.warn_strip.yview, width=12, bg=C_PANEL, troughcolor=C_BG, bd=0)
        ws.pack(side="right", fill="y")
        self.warn_strip.config(yscrollcommand=ws.set)
        ...
        for key, title, col in [("top", "UPPER SENSOR", C_TOP), ("bottom", "LOWER SENSOR", C_BOT)]:
            ...
            dl = tk.Label(card, text="0.000 mm", font=F_DATA, fg=C_GREEN, bg=C_PANEL)
            dl.pack(pady=(8, 2))
            kl = tk.Label(card, text="INTERSECT RATIO: 0.0000", font=F_SMALL, fg=C_TEXT_MED, bg=C_PANEL)
            kl.pack(pady=(0, 8))
            if key == "top":
        ...
        sf = tk.Frame(right, bg=C_BG)
        sf.pack(fill="x", pady=(15, 0))
        self.mv_status_lbl = tk.Label(sf, text="READY FOR INITIAL CAPTURE", font=F_HEAD, fg=C_TEXT_MED, bg=C_BG, wraplength=380, justify="center")
        self.mv_status_lbl.pack(pady=(2, 2))
        
        pf = tk.Frame(right, bg=C_BG)
        pf.pack(fill="x", padx=12)
        self.mv_prog_lbl = tk.Label(pf, text="", font=F_HEAD, fg=C_GREEN, bg=C_BG)
        self.mv_prog_lbl.pack()
        self.mv_prog_bar = ttk.Progressbar(pf, length=380, maximum=COLLECT_N, mode="determinate")
        self.mv_prog_bar.pack(pady=4)

        bf = tk.Frame(right, bg=C_BG)
        bf.pack(pady=10, fill="x", padx=12)
        
        def add_h(b, n, h):
            b.bind("<Enter>", lambda e: b.config(bg=h))
            b.bind("<Leave>", lambda e: b.config(bg=n))
            
        self.btn_reset = tk.Button(bf, text="↺", bg=C_CARD, fg="white", font=F_BTN, relief="flat", width=5, pady=12, bd=0, cursor="hand2", command=self._mv_reset)
        self.btn_reset.pack(side="right", padx=6)
        add_h(self.btn_reset, C_CARD, C_TEXT_MED)
        
        self.btn_start = tk.Button(bf, text="▶ START SESSION", bg=C_GREEN, fg=C_BG, font=F_BTN, relief="flat", padx=20, pady=12, bd=0, cursor="hand2", command=self._mv_start)
        self.btn_start.pack(side="left", expand=True, fill="x", padx=6)
        add_h(self.btn_start, C_GREEN, "#86efac")
        
        self.btn_stop = tk.Button(bf, text="■ STOP (SAVE)", bg=C_RED, fg=C_BG, font=F_BTN, relief="flat", state="disabled", padx=20, pady=12, bd=0, cursor="hand2", command=self._mv_stop)
        self.btn_stop.pack(side="left", expand=True, fill="x", padx=6)
        add_h(self.btn_stop, C_RED, "#fca5a5")
    ...
    def _build_tab_tele(self):
        ...
        for key, title, col in [("top", "TOP", C_TOP), ("bottom", "BOTTOM", C_BOT)]:
            ...
            for v_name in v_show:
                row = tk.Frame(cf, bg=C_PANEL, highlightbackground=C_CARD, highlightthickness=1)
                row.pack(fill="x", padx=10, pady=5)
                tk.Label(row, text=v_name, font=F_HEAD, fg=C_TEXT_MED, bg=C_PANEL).pack(side="left", padx=10, pady=5)
                sv = tk.StringVar(value="—")
                tk.Label(row, textvariable=sv, font=F_MONO, fg=C_TEXT_BRT, bg=C_PANEL).pack(side="right", padx=10, pady=5)
    ...
    def _build_tab_settings(self):
        ...
        rf = tk.LabelFrame(sc, text=" Reference Side (Fixed ArUco) ", bg=C_PANEL, fg=C_TEXT_BRT, font=F_HEAD, padx=20, pady=10)
        ...
        def create_slider(label, var, color, lo, hi, res=0.1):
            f = tk.LabelFrame(sc, text=f" {label} ", bg=C_PANEL, font=F_HEAD, fg=color, padx=25, pady=12)
            f.pack(fill="x", pady=10)
            sl = tk.Scale(f, from_=lo, to=hi, resolution=res, orient="horizontal", variable=var, bg=C_PANEL, fg=C_TEXT_BRT, troughcolor=C_BG, highlightthickness=0, length=600, font=F_BODY)
        ...
        def _update(*_):
            ...
            c_tg.create_oval(4, 4, 76, 36, fill=bg, outline="")
            cx = 58 if on else 22
            c_tg.create_oval(cx-14, 6, cx+14, 34, fill="white", outline="")
            
        tr = tk.Frame(tog_f, bg=C_PANEL); tr.pack(fill="x")
        c_tg = tk.Canvas(tr, width=80, height=40, bg=C_PANEL, highlightthickness=0, cursor="hand2")
        c_tg.pack(side="left", padx=(0, 20))
        t_lbl = tk.Label(tr, text="", font=F_HEAD, bg=C_PANEL, anchor="w")
        t_lbl.pack(side="left", fill="x", expand=True)
        c_tg.bind("<Button-1>", lambda e: (self.use_angle_thresh.set(not self.use_angle_thresh.get()), _update()))
        _update()
    ...
    def _build_tab_cam(self):
        ...
        for m in AWB_MODES: 
            tk.Radiobutton(wr, text=m, variable=self.cam_awb_mode, value=m, bg=C_PANEL, fg=C_TEXT_BRT, selectcolor=C_BG, activebackground=C_PANEL, font=F_SMALL, command=self._apply_cam).pack(side="left", padx=5)
    ...
    def _mv_start(self):
        if self.mv_state in ("idle", "done"):
            ...
            self.mv_status_lbl.config(text=f"Collecting initial... (0 / {COLLECT_N})", fg=C_AMBER)
    ...
    def _mv_stop(self):
        if self.mv_state == "ready":
            ...
            self.btn_stop.config(state="disabled")
            self.mv_prog_bar["value"] = 0
            self.mv_status_lbl.config(text=f"Collecting final... (0 / {COLLECT_N})", fg=C_AMBER)
    ...
    def _mv_reset(self):
        ...
        for il, fl, dl in [ (self.mv_init_lbl_top, self.mv_final_lbl_top, self.mv_delta_lbl_top), (self.mv_init_lbl_bot, self.mv_final_lbl_bot, self.mv_delta_lbl_bot) ]: 
            il.config(text="Init: —", fg=C_TEXT_MED)
            fl.config(text="Final: —", fg=C_TEXT_MED)
            dl.config(text="—", fg=C_ACCENT)
    ...
    def _mv_tick(self):
        ...
        if self.mv_state == "collecting_init":
            ...
            if n >= COLLECT_N:
                ...
                self.mv_init_lbl_top.config(text=f"Init: {self.mv_dist_init['top']:.3f} mm")
                self.mv_init_lbl_bot.config(text=f"Init: {self.mv_dist_init['bottom']:.3f} mm")
        elif self.mv_state == "collecting_final":
            for k in ["top", "bottom"]:
                d = self.last_data[k]["dist"]
                if d > 0:
                    self.mv_final_buf[k].append(d)
            ...
            if n >= COLLECT_N:
                ...
                for key, il, fl, dl in [ ("top", self.mv_init_lbl_top, self.mv_final_lbl_top, self.mv_delta_lbl_top), ("bottom", self.mv_init_lbl_bot, self.mv_final_lbl_bot, self.mv_delta_lbl_bot) ]:
                    di, df = self.mv_dist_init[key], self.mv_dist_final[key]
                    delta = df - di
                    il.config(text=f"Init: {di:.3f} mm")
                    fl.config(text=f"Final: {df:.3f} mm")
    ...
    def _update_telemetry(self):
        for k in ["top", "bottom"]:
            ...
            v["A"].set(f"{d['A'][-1]:.1f}")
            v["X"].set(f"{d['X'][-1]:.1f}")
            v["L_ROT"].set(f"{d['rot_2d']:.1f}°")
            v["L_Z"].set(f"{d['L_z']:.1f}")
            v["L_PITCH"].set(f"{d['L_pitch']:.1f}°")
            v["L_YAW"].set(f"{d['L_yaw']:.1f}°")
            v["R_PITCH"].set(f"{d['R_pitch']:.1f}°")
            v["R_YAW"].set(f"{d['R_yaw']:.1f}°")
            v["R_Z"].set(f"{d['R_z']:.1f}")
    ...
    def _update_warnings(self):
        ...
        for k in ["top", "bottom"]:
            d = self.last_data[k]
            if not d["L_det"]:
                strip.append(f"[{k.upper()}] Left Missing")
            if not d["R_det"]:
                strip.append(f"[{k.upper()}] Right Missing")
            if d["rot_2d"] > r_th:
                strip.append(f"[{k.upper()}] Rotation {d['rot_2d']:.1f}°")
            lp, rp = abs(d["L_pitch"]), abs(d["R_pitch"])
            if lp > p_th:
                strip.append(f"[{k.upper()}] L-Pitch high")
            if rp > p_th:
                strip.append(f"[{k.upper()}] R-Pitch high")
    ...
    def update_gui_loop(self):
        if self.current_frame is not None:
            try: 
                f960 = cv.resize(self.current_frame, (960, 540))
                img = ImageTk.PhotoImage(Image.fromarray(f960))
                self.canvas.create_image(0,0, anchor="nw", image=img); self.canvas.image = img
            except: pass
        if self._cam_preview_frame is not None:
            try: 
                f800 = cv.resize(self._cam_preview_frame, (800, 480))
                img2 = ImageTk.PhotoImage(Image.fromarray(f800))
    ...
    def _apply_cam(self, *_):
        if self.pc:
            ...
            if not self.cam_ae.get():
                c["ExposureTime"] = self.cam_exposure.get()
            try: 
