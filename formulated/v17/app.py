...
from config import *
from gui_widgets import SmartEntry
from logic import measurement_loop
from utils import load_calibration_params

class MeasurementApp:
    """
    Main Application class for the ArUco Measurement System.
    """
    ...
    def _card(self, parent, title, title_color, **pack_kw):
        """Helper to create a stylized LabelFrame."""
    ...
    def _build_tab_live(self):
        """Build the main movement monitoring tab."""
    ...
    def _build_tab_tele(self):
        """Build the telemetry inspection tab."""
    ...
    def _build_tab_settings(self):
        """Build the machine configuration tab."""
    ...
    def _toggle_logic(self, init=False):
        """Toggle between pure v8 logic and fallback logic."""
    ...
    def _build_tab_cam(self):
        ...
        AWB_MODES = {"Auto": 0, "Tungsten": 1, "Fluorescent": 2, "Indoor": 3, "Daylight": 4, "Cloudy": 5}
        for m in AWB_MODES: tk.Radiobutton(mf, text=m, variable=self.cam_awb_mode, value=m, bg=C_PANEL, fg=C_TEXT_MED, font=("Inter", 8), command=self._apply_cam).pack(side="left")
    ...
    def load_calib(self):
        """Interface to utility calibration loader."""
        return load_calibration_params(self.fixed_side.get())

    def _mv_start(self): 
        """Begin initial state collection."""
    ...
    def _mv_reset(self):
        ...
        for lbl_init, lbl_final, lbl_delta in [(self.mv_init_lbl_top, self.mv_final_lbl_top, self.mv_delta_lbl_top), (self.mv_init_lbl_bot, self.mv_final_lbl_bot, self.mv_delta_lbl_bot)]:
            lbl_init.config(text="INIT: —"); lbl_final.config(text="FINAL: —"); lbl_delta.config(text="—")
        self.mv_prog_bar["value"] = 0; self.btn_start.config(state="normal"); self.btn_stop.config(state="disabled")

    def _mv_tick(self):
        """Poll for session completion and update monitor labels."""
        ...
        if self.mv_state == "collecting_init":
            ...
            prog = min(len(self.mv_init_buf["top"]), len(self.mv_init_buf["bottom"]))
            self.mv_prog_bar["value"] = prog
            if prog >= COLLECT_N:
            ...
            prog = min(len(self.mv_final_buf["top"]), len(self.mv_final_buf["bottom"]))
            self.mv_prog_bar["value"] = prog
            if prog >= COLLECT_N:
    ...
    def _apply_cam(self, *_):
        """Apply camera control variables to the hardware."""
        if self.pc is None: return
        import camera
        man = camera.CameraManager(RESOLUTION); man.pc = self.pc; man.apply_controls(self)
    ...
    def _update_side_ui(self):
        """Update radio button highlights based on selected fixed side."""
    ...
    def update_gui_loop(self):
        """Main update loop for canvas and labels."""
        ...
        for k in ["top", "bottom"]:
            ...
            if d["L_det"]:
                if abs(d["L_pit"]) > p_th: strip.append(f"• [{k.upper()}] L-pitch {d['L_pit']:+.1f}°")
                if abs(d["L_yaw"]) > y_th: strip.append(f"• [{k.upper()}] L-yaw {d['L_yaw']:+.1f}°")
            if d["R_det"]:
                if abs(d["R_pit"]) > p_th: strip.append(f"• [{k.upper()}] R-pitch {d['R_pit']:+.1f}°")
                if abs(d["R_yaw"]) > y_th: strip.append(f"• [{k.upper()}] R-yaw {d['R_yaw']:+.1f}°")
