...
class MeasurementApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Dual-Pair ArUco Measurement v14")
        ...
        # Toggle: ON = use angle threshold (fallback to midpoint when rotated)
        #         OFF = always use perpendicular formula regardless of rotation (v8 behaviour)
    ...
    def _build_tab_settings(self):
        ...
        def _draw_toggle():
            ...
            cx = 38 if on else 18
            self._tog_canvas.create_oval(cx-12, 4, cx+12, 24,
                                         fill="white", outline="")
    ...
    def measurement_loop(self):
        ...
        while self.is_running:
            ...
            if ids is not None and len(ids) >= 1:
                ...
                if (curr - l_u) >= 1000:    # 1-second averaging window
                    for key in ["top", "bottom"]:
                        if buffers[key]:
                            s  = buffers[key]   # up to ~10 readings (100ms × 10)
                            ...
                            # Perpendicular ray-intersection (always computed)
                            v_raw = aTR - aTL
                            ...
                            if self.use_angle_thresh.get():
                                # ON: use threshold to switch formula
                                ...
                                # OFF: always perpendicular (v8 mode)
                                aX, dv, kv = _perp()
                            ...
                            # ── CSV + DB: always on every 1s cycle ─────────
                            v_vec = aTR - aTL
                            u_vec = aA
                            w_vec = aC - aB
                            try:
                            ...
                            # ── Image: every 1s from START → STOP ──────────
