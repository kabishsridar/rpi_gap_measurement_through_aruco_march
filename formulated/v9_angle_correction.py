...
class MeasurementApp:
    ...
    def setup_ui(self):
        ...
        self.tabs.add(self.tab_settings, text=" ⚙ Configuration ")
        settings_canvas = tk.Frame(self.tab_settings, bg="#ecf0f1")
        settings_canvas.pack(fill="both", expand=True, padx=100, pady=50)

        tk.Label(settings_canvas, text="ArUco Physical Parameters", font=("Helvetica", 16, "bold"), bg="#ecf0f1", fg="#34495e").pack(pady=(0, 20))
        ...
        def create_size_control(parent, label, var, color):
            ...
            tk.Scale(frame, from_=10.0, to=200.0, resolution=0.1, orient="horizontal", variable=var, bg="white", length=400, highlightthickness=0).pack(side="left", padx=20)
            tk.Entry(frame, textvariable=var, width=8, justify="center").pack(side="left")
        ...
        while self.is_running:
            ...
            if ids is not None and len(ids) >= 2:
                ...
                def process_pair(marker_list, key, size_mm):
                    ...
                    trr, brr = R["pts"][R["pts"][:, 0].argsort()[::-1]][:2][R["pts"][R["pts"][:, 0].argsort()[::-1]][:2][:, 1].argsort()]
                    
                    buffers[key].append({"A":(trl+brl)/2, "B":b, "C":c, "TR_R":trr, "BR_R":brr, "TR":trl, "BR":brl, "TL":tll, "L_Ang":(L["r"], L["t"]), "R_Ang":(R["r"], R["t"])})
            ...
            for key, color in [("top", (0, 165, 255)), ("bottom", (255, 0, 255))]:
                ...
                if d["dist"] > 0:
                    ...
                    if abs(d["L_Ang"][1]) > 5.0:
                        ...
                        cv.putText(frame, f"TILT: {d['L_Ang'][1]:.2f}", (bx+5, by), 0, 0.6, (0,255,255), 2)
