...
class MeasurementApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Dual-Pair ArUco Measurement Dashboard v7")
        self.root.geometry("1500x950")
        self.root.configure(bg="#2c3e50")

        # --- DATA STORAGE ---
        # Separate storage for Top and Bottom pairs
    ...
    def setup_ui(self):
        ...
        # Top Pair Panel
        f_top = tk.LabelFrame(stats_container, text=" TOP PAIR ", font=("Helvetica", 12, "bold"), bg="#ecf0f1", fg="#e67e22")
        f_top.pack(fill="x", pady=10, padx=10)
        self.lbl_dist_top = tk.Label(f_top, text="0.000 mm", font=("Helvetica", 20, "bold"), fg="#27ae60", bg="#ecf0f1")
        self.lbl_dist_top.pack(pady=10)
        self.lbl_k_top = tk.Label(f_top, text="k: 0.000", font=("Helvetica", 10), bg="#ecf0f1")
        self.lbl_k_top.pack()

        # Bottom Pair Panel
        f_bot = tk.LabelFrame(stats_container, text=" BOTTOM PAIR ", font=("Helvetica", 12, "bold"), bg="#ecf0f1", fg="#9b59b6")
        f_bot.pack(fill="x", pady=10, padx=10)
        self.lbl_dist_bot = tk.Label(f_bot, text="0.000 mm", font=("Helvetica", 20, "bold"), fg="#27ae60", bg="#ecf0f1")
        self.lbl_dist_bot.pack(pady=10)
        self.lbl_k_bot = tk.Label(f_bot, text="k: 0.000", font=("Helvetica", 10), bg="#ecf0f1")
        self.lbl_k_bot.pack()

        self.lbl_session = tk.Label(stats_container, text="Logged Points: 0", font=("Helvetica", 10), fg="#7f8c8d", bg="#ecf0f1")
        self.lbl_session.pack(side="bottom", pady=20)

        # TAB 2: TELEMETRY (GRID FOR BOTH)
        ...
        def create_tele_column(parent, title, pair_key, color):
            col_frame = tk.LabelFrame(parent, text=f" {title} DATA ", bg="#ecf0f1", fg=color, font=("Helvetica", 12, "bold"))
            ...
            for var in vars_to_show:
                ...
                self.tele_vars[pair_key][var] = v

        create_tele_column(main_tele_frame, "TOP", "top", "#e67e22")
        create_tele_column(main_tele_frame, "BOTTOM", "bottom", "#9b59b6")

        # TAB 3: FORMULA GUIDE (UNCHANGED)
    ...
    def measurement_loop(self):
        ...
        while self.is_running:
            ...
            if ids is not None and len(ids) >= 2:
                # 1. GROUPING BY Y-COORDS
                # Calculate Y-centers for pairing
                marker_data = []
                for i in range(len(ids)):
                    c_y = np.mean(corners[i][0], axis=0)[1]
                    marker_data.append({"id": ids[i][0], "corners": corners[i][0], "y": c_y})
                
                # Split into Top and Bottom half of image
                top_markers = [m for m in marker_data if m["y"] < RESOLUTION[1]/2]
                bot_markers = [m for m in marker_data if m["y"] >= RESOLUTION[1]/2]

                def process_pair(marker_list, key):
                    if len(marker_list) < 2: return
                    # Sort by X to find Left and Right in the row
                ...
                if (curr - l_sample) >= 100:
                    process_pair(top_markers, "top")
                    process_pair(bot_markers, "bottom")
                    l_sample = curr
